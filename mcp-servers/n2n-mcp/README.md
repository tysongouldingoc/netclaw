...
## Tools (feature 057 — production enforcement & durable runtime)

`N2N_RISK_MODE=production` enforces fail-closed (OpenShell sandbox + DefenseClaw
model-guard/component-scan + GAIT git audit); the Border reports an honest posture
and never claims full production while a control is missing.

| Tool | Purpose |
|------|---------|
| `n2n_posture` | Risk posture: `testing` / `production — enforced` / `production — DEGRADED (<controls>)`, with per-control availability |
| `n2n_faults` | Fault isolation: `daemon` / `member` / `backend` / `none` (if the call itself errors, the daemon is down) |

Durable runtime (mesh daemon + always-on members as `systemd --user` services) is
generated by `scripts/in2n-services.py`.
